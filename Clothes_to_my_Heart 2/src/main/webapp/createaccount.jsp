   	...
   	if(request.getParameter("submit") != null){
   	    ...
   	    String pwd = request.getParameter("password");
   	 	if (userid != null && email != null && pwd != null) {
	   	    ...
	   	    ResultSet rs;
	   	    rs = st.executeQuery("select * from account where username='" + userid + "' or email ='" + email + "'");
	   	    if (rs.next()) {
	   	        out.println("<p>Account already exists with that Username or Email</p>");
	   	    } else {
	   	    	PreparedStatement insertStatement = con.prepareStatement("INSERT INTO account (username, password, email) VALUES (?, ?, ?)");
